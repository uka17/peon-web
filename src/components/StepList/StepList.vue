<template>
  <div class="container">
      ...
      <div class="column">
        <div class="field is-grouped">
          <div class="field has-addons">
            <p class="control">
              <button
                qa-data="move-up-selected-step"
                class="button"
                :disabled="clickedRow === null || clickedRow == 1"
                title="Move step up"
            ...
            <p class="control">
              <button
                qa-data="move-down-selected-step"
                class="button"
                :disabled="clickedRow === null || clickedRow == stepList.length"
    ...
    <p
      id="step-list-empty-error"
      v-if="this.stepList.length === 0"
      class="help is-danger"
    >
      {{ messages.stepListShouldNotBeEmpty["en"] }}
    </p>
    <step
      ref="stepDialog"
      v-on:step-modal-save="save($event)"
      v-on:step-modal-new="create($event)"
    ></step>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    modalEditShow(step) {
      ...
      this.$refs.stepDialog.modalShow(
    ...
    modalNewShow() {
      //Break reactivity for modal edit
      this.$refs.stepDialog.modalShow(
        JSON.parse(JSON.stringify(step_template.newStep)),
        this.stepList,
        true
      );
    },
    ...
    create(newStep) {
      let index =
        this.clickedRow === null ? this.stepList.length : this.clickedRow;
