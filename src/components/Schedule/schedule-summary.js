...
function scheduleSumary(schedule) {
  ...
  if (Object.prototype.hasOwnProperty.call(schedule, "eachNDay")) {
    result = `Each ${schedule.eachNDay} day(s), ${getDailyFrequency(
      schedule.dailyFrequency
    )}, ${getStartEnd(schedule)}`;
  }

  if (Object.prototype.hasOwnProperty.call(schedule, "eachNWeek")) {
    result = `Each ${schedule.eachNWeek} week(s) on ${getWeekDays(
      schedule.dayOfWeek
    )}, ${getDailyFrequency(schedule.dailyFrequency)}, ${getStartEnd(
      schedule
    )}`;
  }

  if (Object.prototype.hasOwnProperty.call(schedule, "month")) {
    result = `In ${getMonths(schedule.month)} each ${getMonthDays(
      schedule.day
    )} day, ${getDailyFrequency(schedule.dailyFrequency)}, ${getStartEnd(
      schedule
    )}`;
  }
...
function getDailyFrequency(dailyFrequency) {
  if (dailyFrequency.hasOwnProperty("occursOnceAt")) {
    ...
    let dailyEnd =
      dailyFrequency.end === undefined ? "23:59:59" : dailyFrequency.end;
...
function listReducer(acc, cur, ind, arr) {
  //x, y, z and a
  let result = `${acc}${cur}`;
  if (arr.length > 2 && ind < arr.length - 2) result = `${result}, `;
  if (ind === arr.length - 2) result = `${result} and `;
