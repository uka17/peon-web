<template>
  <div>
    ...
    <div class="columns">
      <div class="column is-narrow">
        <div class="field has-addons">
          <p class="control">
            <a class="button is-static"> Each </a>
          </p>
          <p class="control">
            <input
              id="schedule-eachnweek"
              type="text"
              qa-data="schedule-dialog-eachnweek"
              v-bind:class="{
                'is-danger':
                  fieldIsValid(
                    'eachNWeek',
                    { 'eachNWeek': eachNWeekValue },
                    eachNWeekConstraints
                  ) !== '',
              }"
              maxlength="3"
              @keypress="isNumber($event)"
              v-model.number="eachNWeekValue"
              class="input"
            />
          </p>
          <p class="control">
        ...
        <div class="field">
          <div class="control">
            <div class="buttons has-addons" qa-data="schedule-dialog-weekday">
              <button
                v-for="(value, index) in reference.weekDayList"
                class="button"
                v-bind:key="index"
                v-bind:id="value.toLowerCase()"
                v-bind:class="{
                  'is-info': weekDayIsToggled(index),
    ...
    <p
      id="schedule-eachnweek-error"
      qa-data="schedule-eachnweek-error"
    ...
    <p
      ...
      qa-data="schedule-weekdays-error"
      class="help is-danger"
    >
      {{
        fieldIsValid(
...
export default {
  data() {
    return {
      ...
      fieldIsValid,
      eachNWeekConstraints: {
        "eachNWeek": constraints("en")["weekly"].eachNWeek,
      },
      dayOfWeekConstraints: {
        "dayOfWeek": constraints("en")["weekly"].dayOfWeek,
      },
