    ...
    <div class="field">
      ...
      <div class="control">
        <div class="buttons has-addons" qa-data="schedule-months">
          <button
            v-for="(value, index) in reference.monthList"
            class="button"
            v-bind:key="index"
            v-bind:id="value.toLowerCase()"
            v-bind:class="{
              'is-info': monthIsToggled(index),
              'is-danger is-outlined':
                fieldIsValid('month', { 'month': month }, monthConstraints) !==
                '',
            }"
      ...
      <p
        id="schedule-months-error"
        qa-data="schedule-months-error"
    ...
    <div class="field">
      ...
      <div class="control">
        <input
          ...
          class="input"
          type="text"
          qa-data="schedule-month-days"
          v-bind:class="{
            'is-danger':
              fieldIsValid('day', { 'day': dayValue }, dayConstraints) !== '',
          }"
          placeholder="Month days comma separated list (e.g. 1,13,29)"
        />
      </div>
      <p
        id="schedule-monthdays-error"
        qa-data="schedule-monthdays-error"
        class="help is-danger"
      >
        {{ fieldIsValid("day", { "day": dayValue }, dayConstraints) }}
      </p>
...
export default {
  ...
  computed: {
    dayValue: {
      ...
      set(newValue) {
        this.$emit("schedule-month-day-update", {
