<template>
  <div>
    <b-field>
      <b-autocomplete
        v-bind:class="{ 'connection-warning': this.connection == null }"
    ...
    <p
      id="step-dialog-connection-error"
      class="help is-danger"
      v-if="this.connection == null"
    >
      {{ fieldIsValid("connection", this.connection, ConnectionConstraints) }}
    </p>
  </div>
</template>
    ...
    async filterConnectionList(value) {
      if (value != null && value != undefined) {
        const result = await axios.get(
          `${this.apiUrl}?filter=${value}&sort=name|asc&page=1&per_page=10`
        );
        if (result.data.data) {
          this.data = result.data.data;
          //check if current value is equal to any connection
          let connection = result.data.data.find((v) => v.name == value);
          if (connection) this.$emit("connection-update", connection.id);
          else this.$emit("connection-update", null);
        }
      }
    },
  },
