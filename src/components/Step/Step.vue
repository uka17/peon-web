    ...
    <div class="modal-card">
      ...
      <section class="modal-card-body">
        <div class="field">
          ...
          <p id="step-dialog-name-error" class="help is-danger">
            {{ fieldIsValid("name") }}
          </p>
        </div>
        <div class="field">
        ...
        >
          ...
          <div
            class="control"
            v-bind:class="{
              'step-command-custom-warning': fieldIsValid('command') !== '',
            }"
          >
            <textarea
              qa-data="step-command"
              class="textarea"
              placeholder="Job description"
              v-bind:class="{ 'is-danger': fieldIsValid('command') !== '' }"
              v-model="step.command"
              id="command-code"
            ></textarea>
          </div>
          <p id="step-dialog-command-error" class="help is-danger">
            {{ fieldIsValid("command") }}
          </p>
        </div>
        ...
        <div class="field has-addons">
          <p class="control">
            <input
              ...
              v-bind:class="{
                'is-danger': fieldIsValid('retryAttempts.number') !== '',
              }"
            />
          </p>
          <p class="control">
            <a
              qa-data="per-each-label"
              class="button is-static"
              v-if="retryAttempts.number !== 0"
            >
              per each
            </a>
            <a
              qa-data="retries-label"
              class="button is-static"
              v-if="retryAttempts.number === 0"
            >
              retries
          ...
          <p class="control" v-if="retryAttempts.number !== 0">
            <input
              ...
              v-bind:class="{
                'is-danger': fieldIsValid('retryAttempts.interval') !== '',
              }"
            />
          </p>
          <p
            qa-data="minutes-label"
            class="control"
            v-if="retryAttempts.number !== 0"
          >
            <a class="button is-static"> minute(s) </a>
          </p>
        </div>
        <p id="step-dialog-retry-number-error" class="help is-danger">
          {{ fieldIsValid("retryAttempts.number") }}
        </p>
        <p id="step-dialog-retry-interval-error" class="help is-danger">
          {{ fieldIsValid("retryAttempts.interval") }}
        </p>
        <div class="field">
      ...
      <footer class="modal-card-foot buttons is-right">
        ...
        <button qa-data="step-cancel" @click="modalClose" class="button">
          Cancel
        </button>
      </footer>
    </div>
...
export default {
    ...
    modalShow(step, stepList, isNew = false) {
      ...
      setTimeout(() => {
        if (this.highlighter === null) {
          this.highlighter = CodeMirror.fromTextArea(
            document.getElementById("command-code"),
            {
              lineNumbers: true,
              theme: "elegant",
            }
  ...
  computed: {
    ...
    retryAttempts: function () {
      return this.step.retryAttempts !== undefined
        ? this.step.retryAttempts
        : {};
    },
    formIsValid() {
