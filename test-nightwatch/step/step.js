...
const testJobTemplate = JSON.parse(
  JSON.stringify(require("../data/jobs.json").job)
);
const testConnections = JSON.parse(
...
describe("step", function () {
  ...
  test.only(`step. 
    ...
    browser
      ...
      .assert.containsText(
        "#step-dialog-retry-number-error",
        "to zero and less than 11"
      )
      .setValue('input[qa-data="step-retry-number"]', [
        "\u0008",
        "\u0008",
      ...
      .assert.containsText(
        "#step-dialog-retry-interval-error",
        "attempt interval must be greater than 0"
      )
      //Put value back
      .setValue(
    ...
    browser
      ...
      .assert.valueContains(
        'div[qa-data="step-result-action-succeed"] select[qa-data="step-result-action"]',
        testJobTemplate.steps[1].onSucceed
      )
      .assert.valueContains(
        'div[qa-data="step-result-action-failure"] select[qa-data="step-result-action"]',
        testJobTemplate.steps[1].onFailure
      )
      .assert.valueContains(
        'div[qa-data="step-dialog-connection"] input',
        testConnections[1].name
      )
      ...
      .click("div.dropdown-content a.dropdown-item")
      .click(
        `div[qa-data="step-result-action-succeed"] select[qa-data="step-result-action"] option[value="${testJobTemplate.steps[1].onFailure}"]`
      )
      .click(
        `div[qa-data="step-result-action-failure"] select[qa-data="step-result-action"] option[value="${testJobTemplate.steps[1].onSucceed}"]`
      )
