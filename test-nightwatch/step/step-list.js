...
let testJobTemplate = JSON.parse(
  JSON.stringify(require("../data/jobs.json").job)
);
  ...
  test(`step-list. 
    ...
    browser
      ...
      .setValue(
        ...
        dayjs(testJobTemplate.schedules[0].oneTime).format(
          "YYYY-MM-DD HH:mm:ss"
        )
      )
      ...
      .assert.attributeContains(
        'button[qa-data="move-down-selected-step"]',
        "disabled",
        "true"
      )
      .assert.attributeContains(
        'button[qa-data="move-up-selected-step"]',
        "disabled",
        "true"
      )
      .assert.attributeContains(
        'button[qa-data="delete-selected-step"]',
        "disabled",
        "true"
      )
      .assert.visible('table[qa-data="step-list"] .vuetable-empty-result')
      .assert.containsText(
        "#step-list-empty-error",
        "Step list should contain at least 1 step"
      )
      ...
      .setValue(
        'input[qa-data="step-retry-number"]',
        testJobTemplate.steps[0].retryAttempts.number
      )
      ...
      .setValue(
        'input[qa-data="step-retry-interval"]',
        testJobTemplate.steps[0].retryAttempts.interval
      )
      .click(
        `div[qa-data="step-result-action-succeed"] select[qa-data="step-result-action"] option[value="${testJobTemplate.steps[0].onSucceed}"]`
      )
      .click(
        `div[qa-data="step-result-action-failure"] select[qa-data="step-result-action"] option[value="${testJobTemplate.steps[0].onFailure}"]`
      )
      .click('button[qa-data="step-create"]')
      //Check if step appeared in table
      .assert.not.elementPresent(
        'table[qa-data="step-list"].vuetable-empty-result'
      )
      .assert.not.elementPresent("#step-list-empty-error")
      .expect.elements('table[qa-data="step-list"] tbody tr')
      .count.to.equal(1);
    //Create 2 more steps
    browser
      //2nd
      ...
      .setValue(
        'input[qa-data="step-retry-number"]',
        testJobTemplate.steps[1].retryAttempts.number
      )
      .setValue('input[qa-data="step-retry-interval"]', [
        "\u0008",
        "\u0008",
      ...
      .setValue(
        'input[qa-data="step-retry-interval"]',
        testJobTemplate.steps[1].retryAttempts.interval
      )
      .click(
        `div[qa-data="step-result-action-succeed"] select[qa-data="step-result-action"] option[value="${testJobTemplate.steps[1].onSucceed}"]`
      )
      .click(
        `div[qa-data="step-result-action-failure"] select[qa-data="step-result-action"] option[value="${testJobTemplate.steps[1].onFailure}"]`
      )
      .click('button[qa-data="step-create"]')
      //3rd
      .click('button[qa-data="create-new-step"')
      ...
      .setValue(
        'input[qa-data="step-retry-number"]',
        testJobTemplate.steps[2].retryAttempts.number
      )
      .setValue('input[qa-data="step-retry-interval"]', [
        "\u0008",
        "\u0008",
      ...
      .setValue(
        'input[qa-data="step-retry-interval"]',
        testJobTemplate.steps[2].retryAttempts.interval
      )
      .click(
        `div[qa-data="step-result-action-succeed"] select[qa-data="step-result-action"] option[value="${testJobTemplate.steps[2].onSucceed}"]`
      )
      .click(
        `div[qa-data="step-result-action-failure"] select[qa-data="step-result-action"] option[value="${testJobTemplate.steps[2].onFailure}"]`
      )
      .click('button[qa-data="step-create"]')
      //Click 1st - Move step and delete controls has correct state and step
