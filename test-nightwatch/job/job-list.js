...
describe("job-list", function () {
  ...
  test("job-list. All components of job list page are visible and has proper state and style", function (browser) {
    browser.assert
      .elementPresent('a[qa-data="job-list-create"]')
  ...
  test("job-list. Line click activates Delete controls and changes line style", function (browser) {
    browser
      .getAttribute(
        'button[qa-data="job-list-delete-modal-show"]',
        "disabled",
        function (result) {
  ...
  test("job-list. Filter control works properly", function (browser) {
    ...
    browser
      ...
      //table has 1 row
      .expect.elements('table[qa-data="job-list-table"] tbody tr')
      .count.to.equal(1);
  ...
  test(`job-list. 
    ...
    browser
      ...
      .assert.elementPresent(
        'a[qa-data="job-delete-modal-confirm"][disabled="disabled"]'
      )
  ...
  test(`job-list. Pagination works properly, changes control style, shown records and statistics of records counter`, function (browser) {
    ...
    .assert
      .elementPresent('div[qa-data="job-list-pagination-info"]')
      .assert.elementPresent('div[qa-data="job-list-pagination"]')
      //check if job is visible
      .assert.elementPresent(`a[qa-data="${testJob.name}"]`)
      //go to next page
      .click(
        'div[qa-data="job-list-pagination"] a[class="btn-nav button"] i[class="mdi mdi-skip-next"]'
      )
      //check if info was changed and job is not visible
      .assert.not.elementPresent(`a[qa-data="${testJob.name}"]`)
      .expect.element('div[qa-data="job-list-pagination-info"]')
      .text.to.contain("Displaying 11");
    browser
      //go back
      .click(
        'div[qa-data="job-list-pagination"] a[class="btn-nav button"] i[class="mdi mdi-skip-previous"]'
      )
      //check if info was changed and job is not visible
      .assert.elementPresent(`a[qa-data="${testJob.name}"]`)
      .expect.element('div[qa-data="job-list-pagination-info"]')
