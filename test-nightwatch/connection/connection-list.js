...
describe('connection-list', function() {
  ...
  test('connection-list. Line click activates Delete controls and changes line style', function (browser) {
    browser
      .getAttribute('button[qa-data="connection-list-delete-modal-show"]', 'disabled', function(result) {
  ...
  test('connection-list. Filter control works properly', function (browser) {
    ...
    browser
      ...
      //table has 1 row
      .expect.elements('table[qa-data="connection-list-table"] tbody tr').count.to.equal(1);

    browser
      //clear filter
      .click('a[qa-data="connection-list-filter-reset"]')
      .getValue('input[qa-data="connection-list-filter-text"]', function(result) {
        this.assert.equal(result.value, '');
      })
      //should be more than 1 row
      .expect.elements('table[qa-data="connection-list-table"] tbody tr').count.to.not.equal(1);
  ...
  `, function (browser) {
    ...
    browser
      ...
      .assert.elementPresent('a[qa-data="connection-delete-modal-confirm"][disabled="disabled"]')
      //put name and check if button enabled
      .click('input[qa-data="connection-delete-modal-text"]')
      .keys([testConnection.name])
      .assert.not.elementPresent('a[qa-data="connection-delete-modal-confirm"][disabled="disabled"]')
      //close with close button
      .click('button[qa-data="connection-delete-modal-close"]')
      .assert.not.cssClassPresent('div[qa-data="connection-delete-modal"]', 'is-active')
  ...
  test(`connection-list. Pagination works properly, changes control style, shown records and statistics of records counter`, function (browser) {
    ...
    browser
      ...
      .assert.elementPresent(`a[qa-data="${testConnection.name}"]`)
      //go to next page
      .click('div[qa-data="connection-list-pagination"] a[class="btn-nav button"] i[class="mdi mdi-skip-next"]')
      //check if info was changed and connection is not visible
      .assert.not.elementPresent(`a[qa-data="${testConnection.name}"]`)      
      .expect.element('div[qa-data="connection-list-pagination-info"]').text.to.contain('Displaying 11');
    browser      
      //go back
      .click('div[qa-data="connection-list-pagination"] a[class="btn-nav button"] i[class="mdi mdi-skip-previous"]')
      //check if info was changed and connection is not visible
      .assert.elementPresent(`a[qa-data="${testConnection.name}"]`)
      .expect.element('div[qa-data="connection-list-pagination-info"]').text.to.contain('Displaying 1')
  });  
