...
const testConnection = JSON.parse(
  JSON.stringify(require("../data/connections.json")[1])
);
...
describe("connection", function () {
  ...
  test("connection. Click to New opens proper modal window. Validation checks", function (browser) {
    browser
      ...
      .assert.cssClassPresent(
        "button[qa-data='connection-dialog-create']",
        "button is-success is-static"
      )
      .assert.cssClassPresent(
        "button[qa-data='connection-dialog-close']",
        "button"
      )
      //name
      .assert.elementPresent('input[qa-data="connection-dialog-name"]')
      .assert.cssClassPresent(
        'input[qa-data="connection-dialog-name"]',
        "input is-danger"
      )
      .assert.containsText(
        'p[qa-data="connection-dialog-name-error"]',
        "including spaces and symbols -_,), not longer than 100 chars"
      )
      ...
      .assert.cssClassPresent(
        'input[qa-data="connection-dialog-host"]',
        "input is-danger"
      )
      .assert.containsText(
        'p[qa-data="connection-dialog-host-error"]',
        "can not be empty"
      )
      ...
      .assert.containsText(
        'p[qa-data="connection-dialog-database-error"]',
        "can not be empty"
      )
      .setValue(
        'input[qa-data="connection-dialog-database"]',
      ...
      .assert.not.cssClassPresent(
        'input[qa-data="connection-dialog-port"]',
        "input is-danger"
      )
      ...
      .assert.not.containsText(
        'p[qa-data="connection-dialog-port-error"]',
        "can not be empty"
      )
      //login
      ...
      .assert.not.containsText(
        'p[qa-data="connection-dialog-password-error"]',
        "can not be empty"
      )
      //type
  ...
  test("connection. Created connection is being reflected in connection list", function (browser) {
    ...
    browser
      ...
      .click(
        `select[qa-data="connection-dialog-type"] option[value="${connection.type}"]`
      )
      //create connection
  ...
  test.only("connection. Change in connection attributes is being reflected in connection list", function (browser) {
    ...
    browser
      ...
      .click(
        `select[qa-data="connection-dialog-type"] option[value="postgresql"]`
      )
      ...
      .assert.valueContains(
        `select[qa-data="connection-dialog-type"]`,
        "postgresql"
      )
      .expect.element('input[qa-data="connection-dialog-enabled"]').to.not.be
      .selected;
  });
  ...
  test("connection. Open existing connection, putting content inside and click Cancel or Close - connection was not changed", function (browser) {
    browser
      .click(`a[qa-data="${testConnection.name}"]`)
      .assert.cssClassPresent(
        'button[qa-data="connection-dialog-save"',
        "button is-link"
      )
      //change values
      .setValue('input[qa-data="connection-dialog-name"]', "-changed")
      ...
      .expect.element('input[qa-data="connection-dialog-enabled"]').to.be
      .selected;
  });
